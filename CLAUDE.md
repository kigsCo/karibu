...
- The UI was originally one ~3,200-line component, `src/KaribuApp.jsx` (14 screens); that monolith has since been fully split and **deleted** (SP1 architecture refactor). The frontend is now route-based: one file per screen under `src/pages/`, shared chrome in `src/layout/` (`AppFrame`/`AppShell`/`FullBleedLayout`), the route table in `src/routes.jsx`, and screens reached by real, deep-linkable URLs (`/b/:slug`, `/guides/:slug`, ...) instead of in-memory nav payloads alone — see `src/CLAUDE.md` for the full structure. Its **data layer is fully migrated**: cities/categories via Context (KAR-5), listings via the keyset-paginated `useBusinesses` hook (KAR-6), business detail + published reviews by slug (KAR-7), review submission through `submit-review` when a session exists (KAR-8), guides via `useGuides` / `useGuideDetail`, and Ask Karibu through the `ask-karibu` proxy. The prototype constants remain as fallbacks — first paint is unchanged and the app still renders if Supabase is unreachable.
- The backend is **scaffolded in-repo** (`supabase/`: 20 migrations, seed, 10 edge functions, 109 Deno tests) and verified end-to-end against the local stack (`supabase start` + `db reset`).
- **A cloud project exists**: `karibu` (`jwiptjcpczamewmyaost`), `eu-central-1`, free plan. Schema + RLS + seed are live and every frontend query is verified against it — see `SUPABASE_SETUP.md`. **All 7 edge functions were deployed to the cloud project on 2026-07-12** (`ask-karibu`, `submit-review`, `moderate-reviews`, `calculate-rankings`, `mpesa-stk-push`, `mpesa-callback`, `send-onboarding-email` — all `ACTIVE`, verified directly against the project). `MIGRATION_CHECKLIST.md` and `SUPABASE_SETUP.md` still read "not deployed" as of this writing and need a follow-up pass to catch up with that fact — secrets and the cron schedule are the remaining Phase 7 configuration work, tracked there. `.env` still points at the local stack for day-to-day dev.
...
- **The business onboarding spine has shipped** (branch `feat/business-onboarding-spine`): the `business-intake` and `admin-review` edge functions back a real register-or-claim flow (`/for-business/register`, `/b/:slug/claim`) with human review at `/admin` and welcome-email activation; staff are appointed with `UPDATE profiles SET is_staff = true`. **Went live 2026-07-23**: all migrations applied to the cloud project, both functions deployed, `INTERNAL_FUNCTION_SECRET` rotated with Vault synced, three crons active, first staff account appointed. Deploys are not automatic on their own — `.github/workflows/deploy.yml`'s `deploy-supabase-functions` job enumerates each function explicitly by name and runs on every push to `main`; it now lists all ten functions (as of this branch), so a function only ships if it's on that list. The approval welcome email still fails closed: `RESEND_API_KEY` is unset and no Karibu sender domain is verified in Resend yet. Cycle 2 (merchant dashboard) is **shipped on branch `feat/merchant-dashboard`**: `/merchant` runs on real owner data with safe-fields self-editing (column-scoped grant, `20260723200000`) and the `merchant-stats` function — the migration and function reach the cloud project on merge to `main` (CI deploys the function; the migration still needs a manual `db push`).
