...
- The backend is **scaffolded in-repo** (`supabase/`: 19 migrations, seed, 9 edge functions, 98 Deno tests) and verified end-to-end against the local stack (`supabase start` + `db reset`).
...
- **The business onboarding spine has shipped** (branch `feat/business-onboarding-spine`): the `business-intake` and `admin-review` edge functions back a real register-or-claim flow (`/for-business/register`, `/b/:slug/claim`) with human review at `/admin` and welcome-email activation; staff are appointed with `UPDATE profiles SET is_staff = true`. **Went live 2026-07-23**: all migrations applied to the cloud project, both functions deployed (and re-deployed automatically by the armed CI on every push to `main`), `INTERNAL_FUNCTION_SECRET` rotated with Vault synced, three crons active, first staff account appointed. The approval welcome email still fails closed: `RESEND_API_KEY` is unset and no Karibu sender domain is verified in Resend yet. Cycle 2 (merchant dashboard) followed: `/merchant` runs on real owner data with safe-fields self-editing (column-scoped grant, `20260723200000`) and the `merchant-stats` function.
