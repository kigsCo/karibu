...
import { UserCircle, Mail, LogOut, Check, Loader2, AlertCircle } from "lucide-react";
  ...
  if (session) {
    return (
      <div className="fade-in flex flex-col items-center px-8 py-16 text-center">
        ...
        <h2 className="font-serif-d text-2xl text-ink mb-1">Your profile</h2>
        <p className="text-sm text-stone-w max-w-xs mb-6">
          Signed in as{" "}
          <span className="text-ink font-medium break-all">{user?.email}</span>
        </p>
        <button
          onClick={signOut}
